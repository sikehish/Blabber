...
def create_normal_report_pdf(meeting_data):
    ...
    if 'screenshots' in meeting_data and meeting_data['screenshots']:
        ...
        for screenshot in meeting_data['screenshots']:
            ...
            try:
                image_path = screenshot['filename']
                img = Image(image_path, width=4*inch, height=3*inch)  # Adjust the size as needed
                elements.append(img)
                elements.append(Spacer(1, 12))
...
def generate_reports(meeting_data):
    ...
    if report_choice == '1':
        ...
        if format_choice == '1':
            create_report_with_interval_sections_pdf(meeting_data, interval)
        elif format_choice == '2':
...
meeting_data = {    
    ...
    'screenshots': [
        {
            'filename': 'screenshot_1727612645290.png',
            'timestamp': '2024-09-29T12:24:05.299Z',
            'takenBy': 'titanyte@gmail.com'
        }
    ]
}
